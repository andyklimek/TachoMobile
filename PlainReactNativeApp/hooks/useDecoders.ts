const useDecoders = () => {
  ...
  const decodeToAscii = (encodedData: number[]): string => {
    ...
    const decodedString = encodedData
      .map(byte => String.fromCharCode(byte)) // Convert each byte to its corresponding ASCII character
      .join(''); // Join all characters into a single string

    // Trim leading \x01 and \x02 characters and trailing whitespace
    return decodedString
      .replace(/^\x01+/, '') // Remove leading \x01
      .replace(/^\x02+/, '') // Remove leading \x02
