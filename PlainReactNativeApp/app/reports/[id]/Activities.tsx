...
const ReportActivities = () => {
  ...
  const faults = reportsData[id]?.driver_activities;

  return (
    <StyledSafeAreaView className="flex-1 bg-lightGray">
      <BackBtn />
      <StyledScrollView contentContainerStyle={{ flexGrow: 1 }} className="px-5">
        <StyledText className="text-3xl text-darkBlue text-center px-3 mb-12">Aktyności</StyledText>
        {faults.length ? (
          faults.map((event, idx) => (
            <Button
              key={idx + 1}
              text={`Aktywność nr. ${idx + 1}`}
