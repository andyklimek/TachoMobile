...
// Done
export class CardMACertificate extends CardFile {
  constructor(sendCommand: (command: number[]) => Promise<number[]>) {
    super(
      'cardma_certificate',
      [0xc1, 0x00],
      0xcc,
      {
        card_certificate: {position: [0, 204], decoder: 'decodeOctetString'},
      },
      false,
      sendCommand,
    );
...
// Done
export class CardSignCertificate extends CardFile {
  constructor(sendCommand: (command: number[]) => Promise<number[]>) {
    super(
      'cardma_certificate',
      [0xc1, 0x01],
...
// Done
export class LinkCertificate extends CardFile {
  constructor(sendCommand: (command: number[]) => Promise<number[]>) {
    super(
      'ca_certificate',
      [0xc1, 0x09],
...
export class VehiclesUsed extends DynamicCardFile {
  ...
  decodeData() {
    ...
    for (let i = 2; i < this.processedData.length; i += 48) {
      const record = this.processedData.slice(i, i + 48);
